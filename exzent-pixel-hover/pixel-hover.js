 ...
 *
 * v1.1 – MutationObserver: re-initialises on dynamically cloned links
 *        (e.g. Flavor Slide Menu copies innerHTML into its panel).
 */
(function () {
        'use strict';
     ...
     /* Store parsed configs so the observer can re-use them */
     var allConfigs = [];
     ...
     ready(function () {
                 ...
                 configs.forEach(function (el) {
                   ...
                   /* ── MutationObserver: watch for new links added to the DOM ─── */
                                   ...
                                   var observer = new MutationObserver(function (mutations) {
                                                       ...
                                                       if (!dominated) return;
                                                                                       ...
                                                                                       allConfigs.forEach(function (cfg) {
                                                                                                               applyEffect(cfg);   // idempotent – skips already-initialised links
                                                                                           });
                                   });
     ...
     function applyEffect(cfg) {
                 ...
                 links.forEach(function (link) {
                                 /* Skip links that are already initialised */
                                           if (link.hasAttribute('data-pixel-hover-init')) return;
                                 link.setAttribute('data-pixel-hover-init', '1');

                                           /* Remove any stale canvas clones (from innerHTML copy) */
                                           var oldCanvases = link.querySelectorAll('.exzent-pixel-layer');
                                 oldCanvases.forEach(function (c) { c.remove(); });

                                           initPixelHover(link, cfg);
                 });
     }
                     ...
                     cells   = [];
                            ...
                            for (var r = 0; r < rows; r++) {
                                                ...
                                                for (var c = 0; c < cols; c++) {
                                                                        var cell = { x: c * cellW, y: r * cellH, col: c, row: r,
                                                                                                                     visible: false, hideAt: 0 };
                                                                        gridRef[r][c] = cell;
                                                                        cells.push(cell);
                                                }
                            }
